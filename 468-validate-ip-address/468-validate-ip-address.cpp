...
public:
    string validIPAddress(string queryIP) {
        ...
        if(regex_match(queryIP, ipv4)) // match regex expression with given IP string for IPv4
            return "IPv4";
        else if(regex_match(queryIP, ipv6)) // match regex expression with given IP string for IPv6
            return "IPv6";
